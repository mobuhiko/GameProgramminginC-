...
void Game::UpdateGame()
{
    ...
    */
    ...
    for (auto& i : mBall)
    {
        i.mBallPos.x += i.mBallVel.x * deltaTime;
        i.mBallPos.y += i.mBallVel.y * deltaTime;
    }
    ...
    for (auto& i : mBall)
    {
        ...
        if (i.mBallPos.y <= thickness && i.mBallVel.y < 0.0f)
            i.mBallVel.y *= -1;
        // the ball collide with the bottom wall?
        else if (i.mBallPos.y >= (768 - thickness) && i.mBallVel.y > 0.0f)
            i.mBallVel.y *= -1;

        /* ball & paddle1 */
        float diff1 = mPaddle1Pos.y - i.mBallPos.y;
        if (diff1 < 0.0f)
            diff1 = -diff1;
        else
            diff1 = diff1;
        ...
        else if (i.mBallPos.x <= 0.0f || i.mBallPos.x >= 1024.0f)
        {
            //mIsRunning = false;
        }
        ...
        if (
            ...
            diff2 <= paddleH / 2.0f &&
            // ball is at the correct x-position
            i.mBallPos.x <= (1024.0f - 10.0f - thickness) && i.mBallPos.x >= 1024.0f - 10.0f - thickness - 5.0f &&
...
void Game::GenerateOutput()
{
    ...
    for (auto& i : mBall)
    {
        SDL_Rect ball
        {
            static_cast<int>(i.mBallPos.x - thickness / 2),
            static_cast<int>(i.mBallPos.y - thickness / 2),
            thickness,
            thickness
        };
